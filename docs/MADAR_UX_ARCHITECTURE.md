...
| البيانات الواصلة | `/workspace/data`: قراءة موحّدة للسجلات التي وصلت فعليًا من المصدر الخارجي |
| المراقبة والتقارير | التحليلات، سجل النشاط |
| المهام | المهام وسير العمل |
| الإدارة | الفريق والصلاحيات، إعدادات النشاط |

السؤال الأساسي في الـOverview: حالة الاتصال، آخر مزامنة، الأخطاء، وما وصل من بيانات. لا تظهر صفحات الكتابة اليدوية الخاصة بـNative في وضع Connected؛ روابطها العميقة تعود بأمان إلى `/workspace/data`، ولا تُعرض بيانات تجريبية.
...
| `/workspace/data` | نفسه | Connected فقط؛ Native يعود إلى Overview |
| صفحات تشغيل Native داخل Connected | `/workspace/data` | redirect سياقي لمنع تعارض مصدر الحقيقة |
