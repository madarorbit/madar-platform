...
import {
 SupabaseMadarIntegrationContextReader,SupabaseMadarToolGateway,SupabaseOrbyExecutionQueue,SupabaseOrbyExecutionRepository,
 SupabaseOrbyMembershipResolver,
...
class KernelPlanningModel implements OrbyPlanningModel {
 ...
 async generatePlan(input:{goal:string;identity:OrbyIdentity;reason:string;tools:readonly OrbyToolManifest[];signal?:AbortSignal}){
  const toolSummary=input.tools.map(tool=>({name:tool.name,description:tool.description,category:tool.category,operation:tool.operation,riskLevel:tool.riskLevel,inputSchema:tool.inputSchema,requiredPermissions:tool.requiredPermissions}));
  const response=await this.kernel.execute({identity:input.identity,message:`أنت مخطط مهام ORBY فقط. لا تنفذ أي أداة ولا تدّع التنفيذ. حوّل الهدف إلى خطة JSON صالحة وتستخدم حصراً أسماء الأدوات المرفقة.\n\nالهدف: ${input.goal}\nسبب التنفيذ: ${input.reason}\nالأدوات: ${JSON.stringify(toolSummary)}\n\nأعد كائن JSON فقط بهذه الحقول: id، goal، summary، root، toolNames، dependencies، riskLevel، terminalCondition، createdAt، planner. root عقدة من الأنواع action أو sequence أو parallel أو condition أو loop أو delay أو approval أو event. كل action يجب أن يحمل id وtype=action وtoolName وinput. لا تضف أداة غير موجودة، ولا تضع بيانات سرية، ولا تتجاوز الموافقات.`,requiredCapabilities:['text'],signal:input.signal,metadata:{purpose:'agent-planning',noToolExecution:true}});return response.text;
 }
}
...
async function buildServerRuntime(){
 ...
 await repository.syncToolCatalog(MADAR_EXECUTION_TOOL_MANIFESTS);const enabledNames=await repository.enabledToolNames(),registry=new OrbyToolRegistry();await new OrbyToolLoader(registry).load(createMadarExecutionTools(gateway,enabledNames).map(tool=>()=>tool));
 ...
 return{runtime,foundation,repository,queue,registry,events};
}
