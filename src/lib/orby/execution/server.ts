...
async function buildServerRuntime(){const database=new IntegrationDatabase(),repository=new SupabaseOrbyExecutionRepository(database),queue=new SupabaseOrbyExecutionQueue(database),memberships=new SupabaseOrbyMembershipResolver(database),events=new InMemoryExecutionEventBus(),contextReader=new SupabaseMadarIntegrationContextReader(database);const foundation=await createServerOrbyFoundation({contextSources:[new MadarIntegrationContextSource(contextReader)]});const gateway=new SupabaseMadarToolGateway(database,async input=>{const response=await foundation.kernel.execute({identity:input.identity,sessionId:input.sessionId,message:input.prompt,requiredCapabilities:['text'],signal:input.signal,metadata:{purpose:'agent-tool-analysis',toolsDisabled:true}});return{text:response.text,sessionId:response.sessionId,providerId:response.providerId,modelId:response.modelId};});await repository.syncToolCatalog(MADAR_EXECUTION_TOOL_MANIFESTS);const enabledNames=await repository.enabledToolNames(),registry=new OrbyToolRegistry();await new OrbyToolLoader(registry).load(createMadarExecutionTools(gateway,enabledNames).map(tool=>()=>tool));const manifestPlanner=new ManifestTaskPlanner(registry),planner=new CompositeTaskPlanner([new ModelTaskPlanner(new KernelPlanningModel(foundation.kernel),registry),manifestPlanner]),permissions=new OrbyPermissionEngine(memberships),runtime=createOrbyAgentRuntime({planner,registry,repository,queue,permissions,policies:new OrbyPolicyEngine(),events});attachOrbyOsObservability(events);return{runtime,foundation,repository,queue,registry,events};}
