...
function AuthenticatedApp({ session }: { session: Session }) {
  ...
  if (loading && !snapshot)
    return <LoadingScreen label="جارٍ تجهيز مركز القيادة…" />;
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.app}>
        ...
        <BottomNav
          active={tab}
          onChange={setTab}
          attention={snapshot.alerts.some(
            (item) =>
      ...
      >
        ...
        >
          ...
          <Text style={styles.loginBody}>
            اطّلع على حالة تجارتك، المؤشرات والتنبيهات الذكية دون أدوات تعديل أو
            صلاحيات حساسة.
          </Text>
          ...
          <Text style={styles.securityNote}>
            تُحفظ الجلسة محليًا وتُطبق صلاحيات مساحة العمل نفسها الموجودة في
            منصة مَدار.
          </Text>
        </ScrollView>
      </KeyboardAvoidingView>
  ...
  return (
    <ScreenScroll refreshing={refreshing} onRefresh={onRefresh}>
      <View style={styles.screenHeader}>
        <View style={styles.headerTextWrap}>
          ...
          <Text style={styles.title}>
            {snapshot.profile.fullName || "عميل مَدار"}
          </Text>
      ...
      {(offline || error) && (
        <View style={styles.offlineBanner}>
          <Text style={styles.offlineText}>
            {error || "أنت في وضع العرض المحفوظ."}
          </Text>
        </View>
      )}
      ...
      <View
        style={[
          styles.commandCard,
          needsAttention ? styles.commandAttention : styles.commandHealthy,
        ]}
      >
        ...
        <View style={styles.commandCopy}>
          ...
          <Text style={styles.commandTitle}>
            {needsAttention
              ? `${snapshot.alerts.length} أمور تستحق انتباهك`
              : "كل شيء يبدو مستقرًا"}
          </Text>
          <Text style={styles.commandBody}>
            {needsAttention
              ? "رتّب أوربي التنبيهات حسب الأولوية لتعرف ما الذي تراجعه أولًا."
              : "لا توجد تنبيهات حرجة في البيانات المتاحة حاليًا."}
          </Text>
        </View>
      </View>
      ...
      <View style={styles.metricsGrid}>
        <MetricCard
          label="المبيعات"
          value={money(summary.revenue30d, workspace.currency)}
          accent="mint"
        />
        <MetricCard
          label="صافي تقريبي"
          value={money(summary.profit30d, workspace.currency)}
          accent={summary.profit30d >= 0 ? "violet" : "red"}
        />
        <MetricCard
          label="مبيعات اليوم"
          value={money(summary.todayRevenue, workspace.currency)}
          accent="sky"
        />
        <MetricCard
          label="العملاء"
        ...
        <MetricCard
          ...
          accent={summary.lowStock ? "amber" : "mint"}
        />
        <MetricCard
      ...
      <SectionTitle
        title="ما يستحق انتباهك"
        hint={`${snapshot.alerts.length} تنبيه`}
      />
      <View style={styles.stack}>
        {snapshot.alerts.length ? (
          ...
          <EmptyCard text="لا توجد تنبيهات نشطة الآن." />
        )}
      </View>
            ...
            <View key={task.id} style={styles.listCard}>
              <View style={styles.listMain}>
                ...
                <Text style={styles.listMeta}>
                  {task.dueAt ? dateTime(task.dueAt) : "بلا موعد محدد"}
                </Text>
              </View>
              <Pill
      ...
      <Text style={styles.lastUpdate}>
        آخر تحديث: {dateTime(snapshot.fetchedAt)}
      </Text>
    </ScreenScroll>
  );
}
...
function ReportsScreen({
  ...
  const maxValue = Math.max(
    1,
    ...snapshot.dailySeries.flatMap((item) => [item.revenue, item.expenses]),
  );
  ...
  return (
    <ScreenScroll refreshing={refreshing} onRefresh={onRefresh}>
      <PageHeader
        eyebrow="التقارير"
        title="الأداء المالي"
        subtitle="قراءة مختصرة من بيانات مساحة العمل"
      />
      <View style={styles.reportHero}>
        ...
        <Text
          style={[
            ...
            snapshot.summary.profit30d < 0 && styles.negative,
          ]}
        >
          {money(snapshot.summary.profit30d, currency)}
        </Text>
        <View style={styles.reportSplit}>
          <MiniValue
            label="الإيرادات"
            value={money(snapshot.summary.revenue30d, currency)}
          />
          <MiniValue
            label="المصروفات"
            value={money(snapshot.summary.expenses30d, currency)}
          />
        </View>
      </View>
              ...
              <View style={styles.barTrack}>
                <View
                  style={[
                    ...
                    styles.revenueBar,
                    {
                      width: `${Math.max(2, (point.revenue / maxValue) * 100)}%`,
                    },
                  ]}
                />
                <View
                  style={[
                    ...
                    styles.expenseBar,
                    {
                      width: `${Math.max(2, (point.expenses / maxValue) * 100)}%`,
                    },
                  ]}
                />
              </View>
              <Text style={styles.chartNumber}>
                {Math.round(point.revenue).toLocaleString("ar-YE")}
              </Text>
            </View>
          ))}
        </View>
      ...
      <SectionTitle
        title="آخر المبيعات"
        hint={`${snapshot.recentSales.length} عمليات`}
      />
      <View style={styles.stack}>
...
function OrbyScreen({
  ...
  async function send() {
    ...
    try {
      ...
      const message =
        sendError instanceof Error ? sendError.message : "تعذر تشغيل أوربي.";
      setMessages((current) => [
        ...current,
        {
          id: `error-${Date.now()}`,
    ...
    >
      ...
      >
        <PageHeader
          ...
          title="أوربي"
          subtitle="يسأل بيانات مَدار ويقدّم تفسيرًا وخطوة تالية"
        />
        <View style={styles.orbyStatus}>
          <View style={styles.orbyOrb}>
            <Text style={styles.orbyOrbText}>O</Text>
          </View>
          <View style={styles.orbyStatusCopy}>
            <Text style={styles.orbyStatusTitle}>
              متصل بمساحة {snapshot.workspace.name}
            </Text>
            <Text style={styles.orbyStatusBody}>
              لا يملك التطبيق أدوات تعديل، وأي إجابة مبنية على صلاحيات حسابك
              فقط.
            </Text>
          </View>
        </View>
        ...
        <View style={styles.chatArea}>
          {!messages.length && (
            <View style={styles.promptIdeas}>
              ...
              {[
                "كيف كان أداء المبيعات هذا الأسبوع؟",
                "ما الذي يستحق انتباهي في المخزون؟",
                "اقترح أولوياتي لليوم.",
              ].map((idea) => (
                <Pressable
                  key={idea}
          ...
          {messages.map((message) => (
            ...
            >
              <Text style={styles.messageRole}>
                {message.role === "user" ? "أنت" : "أوربي"}
              </Text>
              <Text style={styles.messageText}>{message.text}</Text>
              {message.source === "smart-fallback" && (
                <Text style={styles.fallbackLabel}>تحليل احتياطي آمن</Text>
              )}
            </View>
          ))}
          {sending && (
            <View style={[styles.message, styles.orbyMessage]}>
        ...
        <View style={styles.composer}>
          ...
          <Pressable
            onPress={send}
            disabled={sending || prompt.trim().length < 5}
            style={({ pressed }) => [
              styles.sendButton,
              (sending || prompt.trim().length < 5) && styles.disabled,
        ...
        {remaining !== null && (
          <Text style={styles.remaining}>المتبقي اليوم: {remaining} طلبًا</Text>
        )}
      </ScrollView>
    </KeyboardAvoidingView>
  );
...
function AccountScreen({
  ...
  return (
    ...
    >
      <PageHeader
        eyebrow="الحساب"
        title={snapshot.profile.fullName || "حساب مَدار"}
        subtitle={snapshot.profile.email || session.user.email || ""}
      />
      <View style={styles.profileCard}>
        <View style={styles.avatar}>
          <Text style={styles.avatarText}>
            {(snapshot.profile.fullName || snapshot.profile.email || "م")
              .trim()
              .charAt(0)}
          </Text>
        </View>
        ...
        <Text style={styles.profileEmail}>
          {snapshot.profile.email || session.user.email}
        </Text>
      </View>
      ...
      <InfoRow
        label="الاشتراك"
        value={subscriptionLabel(snapshot.subscriptionStatus)}
      />
      <View style={styles.readOnlyCard}>
        <Text style={styles.readOnlyTitle}>تطبيق عرض آمن</Text>
        <Text style={styles.readOnlyBody}>
          لا يتضمن هذا التطبيق إنشاء منتجات أو تعديل مخزون أو إدارة مستخدمين.
          تتم العمليات الإدارية من منصة الويب فقط.
        </Text>
      </View>
      <GhostButton
        label="تسجيل الخروج"
        onPress={() =>
          Alert.alert("تسجيل الخروج", "هل تريد إنهاء الجلسة على هذا الجهاز؟", [
      ...
      {items.map((item) => (
        ...
        >
          <View
            style={[
              styles.navIconWrap,
              active === item.key && styles.navIconWrapActive,
            ]}
          >
            <Text
              style={[
                styles.navIcon,
                active === item.key && styles.navIconActive,
              ]}
            >
              {item.icon}
            </Text>
            {item.key === "home" && attention && (
          ...
          <Text
            style={[
              styles.navLabel,
              active === item.key && styles.navLabelActive,
            ]}
          >
            {item.label}
          </Text>
...
function MetricCard({
  ...
  label: string;
  value: string;
  accent: "mint" | "violet" | "amber" | "red" | "sky";
}) {
  const accentStyle = {
...
function Pill({ text, tone }: { text: string; tone: "danger" | "neutral" }) {
  return (
    <View style={[styles.pill, tone === "danger" && styles.pillDanger]}>
      <Text
        style={[styles.pillText, tone === "danger" && styles.pillDangerText]}
      >
        {text}
      </Text>
    </View>
...
function MiniValue({ label, value }: { label: string; value: string }) {
  return (
    <View style={styles.miniValue}>
      ...
      <Text style={styles.miniNumber}>{value}</Text>
    </View>
  );
}
...
const modeLabel = (value: OrbyMode) =>
  ({
    GENERAL: "عام",
    SALES: "المبيعات",
    INVENTORY: "المخزون",
...
const workspaceType = (value: string) =>
  ({ INDIVIDUAL: "فرد", MERCHANT: "متجر", COMPANY: "شركة", STUDENT: "طالب" })[
    value
  ] || value;
const roleLabel = (value: string) =>
  ({ OWNER: "المالك", ADMIN: "مدير", MEMBER: "عضو", VIEWER: "مشاهد" })[value] ||
  value;
const subscriptionLabel = (value: DashboardSnapshot["subscriptionStatus"]) =>
  ({
    active: "نشط",
    past_due: "متأخر",
    expired: "منتهي",
...
const styles = StyleSheet.create({
  ...
  offlineBanner: {
    ...
    borderWidth: 1,
    borderColor: "rgba(247,200,115,.25)",
    padding: 12,
    borderRadius: 14,
    marginTop: 16,
  },
  ...
  commandHealthy: {
    ...
    borderColor: "rgba(112,228,212,.25)",
  },
  commandAttention: {
    backgroundColor: colors.amberSoft,
    borderColor: "rgba(247,200,115,.25)",
  ...
  alertCritical: {
    ...
    borderColor: "rgba(251,113,133,.24)",
  },
  alertWarning: {
    backgroundColor: colors.amberSoft,
    borderColor: "rgba(247,200,115,.24)",
  },
  alertInfo: {
  ...
  alertSuccess: {
    backgroundColor: colors.mintSoft,
    borderColor: "rgba(112,228,212,.22)",
  },
  alertTitle: {
    color: colors.text,
    fontSize: 14,
  ...
  reportHero: {
    borderRadius: 24,
    borderWidth: 1,
    borderColor: "rgba(155,123,255,.25)",
    backgroundColor: colors.violetSoft,
    padding: 20,
  },
  reportHeroLabel: {
  ...
  userMessage: {
    ...
    backgroundColor: colors.violetSoft,
    borderWidth: 1,
    borderColor: "rgba(155,123,255,.22)",
  },
  orbyMessage: {
  ...
  avatar: {
    ...
    backgroundColor: colors.violetSoft,
    borderWidth: 1,
    borderColor: "rgba(155,123,255,.3)",
    alignItems: "center",
    justifyContent: "center",
  },
  ...
  bottomNav: {
    ...
    borderWidth: 1,
    borderColor: colors.border,
    backgroundColor: "rgba(17,21,29,.98)",
    flexDirection: "row-reverse",
    alignItems: "center",
