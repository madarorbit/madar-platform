...
export default function OrbyChat({authenticated,organizationId,contextLabel,initialConversationId,initialMessages,initialRemaining,initialLimit,tier:initialTier,starter,historyLimited=false}:Props){
 ...
 async function copyMessage(message:Message){try{await navigator.clipboard.writeText(message.content);setCopiedId(message.id);window.setTimeout(()=>setCopiedId(current=>current===message.id?null:current),1600);}catch{setStatus('تعذر النسخ تلقائيًا. حدّد النص وانسخه يدويًا.');window.setTimeout(()=>setStatus(''),2200);}}
 return <div className="md-orby-chat" aria-busy={busy}>
  ...
  <div className="md-orby-composer-region"><div className="md-orby-composer-wrap">{!online?<div className="md-orby-offline" role="status"><Icon name="warning"/>لا يوجد اتصال. يمكنك متابعة كتابة المسودة، ولن تُرسل حتى يعود الإنترنت.</div>:null}{blocked?<div className="md-orby-limit"><p>{authenticated?'استخدمت رسائل ORBY المتاحة اليوم. يمكنك الترقية إلى ORBY Plus للمتابعة.':'استخدمت 5 رسائل ORBY المتاحة للزائر اليوم.'}</p><Link href={authenticated?'/orby/plus':'/register?next=/orby'}>{authenticated?'الترقية إلى ORBY Plus':'أنشئ حسابًا للاستمرار'}</Link></div>:null}<form onSubmit={event=>{event.preventDefault();void send();}} className="md-orby-composer"><textarea ref={textareaRef} value={prompt} onChange={event=>setPrompt(event.target.value)} onKeyDown={event=>{if(event.key==='Enter'&&(event.ctrlKey||event.metaKey)&&!event.nativeEvent.isComposing){event.preventDefault();void send();}}} disabled={blocked} rows={1} maxLength={12000} placeholder={online?'اكتب لأوربي…':'اكتب مسودتك… ستُرسل بعد عودة الاتصال'} aria-label="رسالتك إلى ORBY" aria-describedby="orby-composer-help"/><div className="md-orby-composer-actions"><span id="orby-composer-help">Enter لسطر جديد · Ctrl/⌘+Enter للإرسال</span><div className="md-orby-composer-tools" data-voice-ready="true">{busy?<IconButton label="إيقاف التوليد" onClick={stop}><span className="md-orby-stop-icon"/></IconButton>:<button type="submit" disabled={!prompt.trim()||blocked||!online} className="md-orby-send" aria-label="إرسال"><Icon name="send"/></button>}</div></div></form><p className="md-orby-disclaimer">قد يخطئ ORBY. راجع القرارات المهمة، ولا يملك وصولًا إلا للسياق المصرح به.</p></div></div>
 </div>;
}
