...
export default function OrbyShell({children,sidebar,plus,newChatHref,authenticated,contextLabel,returnHref,identity,shellContext}:{children:ReactNode;sidebar?:ReactNode;plus:boolean;newChatHref:string;authenticated:boolean;contextLabel:string;returnHref:string;identity?:ShellIdentity;shellContext?:ShellContextDefinition;}) {
  ...
  const shell = (
    <main className="md-orby-shell">
        ...
        <header className="md-orby-header md-no-print" data-madar-guide-occluder="top">
          <div className="md-orby-header-main">
            ...
            {authenticated&&sidebar?<>
              <IconButton label="فتح محادثات ORBY" className="md-orby-mobile-sidebar-button" onClick={()=>setSidebarOpen(true)} aria-expanded={sidebarOpen}><Icon name="menu" /></IconButton>
              <IconButton label={sidebarCollapsed?"إظهار محادثات ORBY":"إخفاء محادثات ORBY"} className="md-orby-desktop-sidebar-button" onClick={()=>setSidebarCollapsed(value=>!value)} aria-expanded={!sidebarCollapsed} aria-controls="orby-conversation-sidebar"><Icon name="menu" /></IconButton>
            </>:null}
          ...
          <div className="md-orby-header-actions">
            <ButtonLink href={newChatHref} aria-label="بدء محادثة ORBY جديدة" className="md-orby-new-chat" variant="secondary" size="sm" onClick={authenticated?undefined:(event)=>{event.preventDefault();window.location.assign(`/orby?conversation=new&session=${crypto.randomUUID()}`);}}><Icon name="plus" /><span>محادثة جديدة</span></ButtonLink>
            {authenticated&&identity?<GlobalUserActions displayName={identity.displayName} hasAvatar={identity.hasAvatar} isAdmin={identity.isAdmin} unread={identity.unread} notifications={identity.notifications} showOrby={false}/>:<><CartStatusLink/><ButtonLink href="/login?next=/orby" variant="ghost" size="sm">تسجيل الدخول</ButtonLink></>}
            <Menu label="خيارات ORBY" trigger={<span className="md-icon-button"><Icon name="more" /></span>}>
              ...
              {authenticated?<><Link href="/account/privacy"><Icon name="shield" />الخصوصية والبيانات</Link><Link href="/account"><Icon name="user" />حساب مَدار</Link></>:null}
            </Menu>
          </div>
        </header>
      ...
      {authenticated&&sidebar?<Sheet open={sidebarOpen} onClose={closeSidebar} title="محادثات ORBY" description={contextLabel}>{sidebar}</Sheet>:null}
