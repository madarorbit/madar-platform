...
export default function EnterpriseWorkspaceShell({
  ...
  children: ReactNode; workspaceName: string; role: string; currency: string; subscriptionStatus: string; extension: VerticalExtension; specializationName: string; enabledModules: string[]; operatingMode: OperatingMode; initialCompact: boolean;
}) {
  ...
  const changeCompact = () => setCompact((value) => {
    ...
    startTransition(() => { void saveWorkspaceNavigationState(next); });
    return next;
  });
  const isActive = (href: string) => href === "/workspace" ? pathname === href : pathname.startsWith(href);

  const navigation = (mobile = false) => <nav className={mobile ? "md-mobile-drawer-nav" : "md-ux-sidebar-nav"} aria-label="تنقل مساحة العمل">
  ...
  return <div className={cx("md-ux-shell", compact && "is-compact")}> 
    <aside className="md-ux-sidebar md-no-print">
      <div className="md-workspace-switcher">
        ...
        <details>
          <summary><span className="md-workspace-avatar">{workspaceName.slice(0, 1)}</span><span className="md-workspace-switcher-copy"><strong>{workspaceName}</strong><small>{specializationName}</small></span><Icon name="arrow" className="h-3.5 w-3.5" /></summary>
      ...
      <div className="md-ux-sidebar-footer">
        <button type="button" onClick={changeCompact} className="md-sidebar-control"><Icon name="layers" /><span>{compact ? "توسيع التنقل" : "تنقل مدمج"}</span></button>
        <span className="md-sidebar-footnote">{currency} · بيانات المساحة معزولة</span>
      </div>
    </aside>
    ...
    <div className="md-ux-main">
      <header className="md-ux-topbar md-no-print">
        ...
        <div className="md-topbar-actions">
          <ThemeToggle />
          <Link href="/workspace/orby" className="md-orby-topbar"><Image src={siteConfig.assets.orby} alt="أوربي" width={28} height={28} unoptimized /><span>اسأل أوربي</span></Link>
    ...
    <nav className="md-mobile-bottom-nav md-no-print" aria-label="التنقل السريع">
      {mobileItems.map((item) => <Link key={item.href} href={item.href} aria-current={isActive(item.href) ? "page" : undefined} className={cx(isActive(item.href) && "is-active", item.orby && "is-orby")}>
        {item.orby ? <Image src={siteConfig.assets.orby} alt="" width={34} height={34} unoptimized /> : <Icon name={item.icon} className="h-5 w-5" />}<span>{item.label.replace(" والتقارير", "")}</span>
      </Link>)}
