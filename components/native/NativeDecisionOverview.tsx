...
const number = new Intl.NumberFormat("ar-YE", { maximumFractionDigits: 2 });
const integer = new Intl.NumberFormat("ar-YE", { maximumFractionDigits: 0 });
...
function metricValue(result: NormalizedMetricResult) {
  ...
  if (result.unit.kind === "percentage") return `${number.format(result.value)}٪`;
  if (result.unit.kind === "duration") {
    ...
    return `${number.format(result.value)} ${unit}`;
  }
  return result.unit.kind === "count" ? integer.format(result.value) : number.format(result.value);
}
...
function setupCopy(context: NativeOverviewContext) {
  const core = nativeCoreModule(context.extension);
  ...
  return `وحدة التشغيل الأساسية (${core}) غير مفعلة. لا تعرض مَدار مؤشرات أو أصفارًا لوحدة غير متاحة.`;
}

function firstUseCopy(vertical: NativeVertical) {
  if (vertical === "food_service") return { title: "ابدأ تشغيل المطعم", description: "لا توجد وصفات أو طلبات تشغيلية بعد. أنشئ أول وصفة ثم ابدأ أول طلب حقيقي.", href: "/workspace/restaurant", label: "فتح تشغيل المطعم" };
  if (vertical === "hospitality") return { title: "ابدأ إعداد الفندق", description: "لا توجد منشآت أو غرف تشغيلية بعد. أضف أول منشأة وغرفة لبدء قياس الإشغال والتشغيل.", href: "/workspace/hotel", label: "فتح تشغيل الفندق" };
...
function quickActions(context: NativeOverviewContext) {
  ...
  return actions.filter((action) => !action.module || context.enabledModules.includes(action.module));
}

function CurrentAndSupporting({ model }: { model: NativeOverviewModel }) {
  return <>
    {model.current.length ? (
      <DashboardSection eyebrow="التشغيل" title="الحالة التشغيلية الحالية" description="Current State فقط؛ هذه القيم لا تتظاهر بأنها تتبع فترة زمنية عالمية." priority="normal">
        <DashboardMetricGrid>{model.current.map(metricCard)}</DashboardMetricGrid>
      </DashboardSection>
    ) : null}
    {model.supporting.length ? (
      <DashboardSection eyebrow="السياق" title="معلومات داعمة للقرار" description="مؤشرات مساندة فقط، وليست طبقة KPIs رئيسية إضافية." priority="supporting">
        <DashboardMetricGrid>{model.supporting.map(metricCard)}</DashboardMetricGrid>
      </DashboardSection>
    ) : null}
  </>;
}

export async function NativeDecisionOverview({
  workspace,
  sector,
  moduleUnavailable,
}: {
  ...
  const starter = firstUseCopy(context.extension);
  ...
  return <WorkspaceModule>
    <WorkspaceModuleHeader
      ...
      icon={sector.extension === "hospitality" ? "home" : "store"}
      actions={<><ButtonLink href="/workspace/orby" variant="secondary"><Icon name="sparkles" />اسأل ORBY</ButtonLink><ButtonLink href={action.href}><Icon name={action.icon} />{action.label}</ButtonLink></>}
    />
    ...
    {model.notices.length ? (
      <DashboardSection eyebrow="الثقة" title="سياق يجب معرفته قبل قراءة الأرقام" description="تظهر هذه الرسائل عندما لا يمكن توحيد العملة أو الزمن بأمان؛ لا تنفذ مَدار FX أو افتراضات زمنية ضمن Phase 7.">
        <div className="grid gap-3">{model.notices.map((notice) => <DashboardStatusBlock key={`${notice.href}-${notice.title}`} title={notice.title} description={notice.description} tone={notice.tone} action={<DashboardDrillDownLink href={notice.href}>فتح التفاصيل</DashboardDrillDownLink>} />)}</div>
      </DashboardSection>
    ) : null}

    {model.isFirstUse ? (
      <DashboardSection eyebrow="البدء" title={starter.title} description="هذه حالة First-use حقيقية وليست بديلًا عن صفر تشغيلي أثناء وجود نشاط." priority="primary">
        <DashboardEmptyState title={starter.title} description={starter.description} icon={context.extension === "hospitality" ? "home" : "store"} action={<ButtonLink href={starter.href}>{starter.label}</ButtonLink>} />
      </DashboardSection>
    ) : <>
      ...
      {model.attention.length ? (
        <DashboardSection eyebrow="الانتباه" title="ما الذي يحتاج تدخلك الآن؟" description="تظهر فقط حقائق Domain تستحق إجراءً؛ الحالات التشغيلية العادية لا تتحول إلى Alerts.">
          <div className="grid gap-3">{model.attention.map(attentionBlock)}</div>
        </DashboardSection>
      ) : null}
