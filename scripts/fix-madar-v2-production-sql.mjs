...
if(!after.includes('select o.organization_id,count(*) filter')||!after.includes('filter(where o.status not in')||!after.includes('group by o.organization_id;'))throw new Error('MADAR_V2_RESTAURANT_REPORT_PATCH_INVALID');
await writeFile(path,source.slice(0,start)+after+source.slice(end),'utf8');
console.log('Qualified restaurant profit report organization_id and status references.');
