...
export async function submitWorkspacePayment(
  ...
  try {
    ...
    return {
      success:
        "تم إرسال إثبات التحويل للمراجعة. ستفتح المساحة تلقائياً بعد موافقة الإدارة.",
      destination: "/dashboard",
    };
  } catch (error) {
    return {
      error:
        error instanceof Error ? error.message : "تعذر إرسال بيانات الدفع.",
    };
  }
}

export async function reviewWorkspaceRequest(
  _previous: WorkspaceActionState,
  form: FormData,
): Promise<WorkspaceActionState> {
  try {
    ...
    return {
      success:
        decision === "approve"
          ? "تم قبول الطلب وفتح المساحة تلقائياً."
          : "تم رفض الطلب.",
    };
  } catch (error) {
    return {
      error: error instanceof Error ? error.message : "تعذر مراجعة الطلب.",
    };
  }
}
...
export async function updateWorkspace(_previous: unknown, form: FormData) {
  try {
    ...
    if (!organizationTypes.includes(type as never))
      throw new Error("نوع الحساب غير صالح.");
    const rows = await supabaseFetch(
      `/rest/v1/organization_members?organization_id=eq.${encodeURIComponent(id)}&user_id=eq.${encodeURIComponent(user.id)}&select=role`,
    );
    if (rows?.[0]?.role !== "OWNER")
      throw new Error("مالك المساحة فقط يستطيع تعديل إعداداتها.");
    await supabaseFetch(
...
export async function manageWorkspaceMember(
  ...
  try {
    ...
    const requestedRole = String(form.get("role") || "MEMBER") as
      "MEMBER" | "ADMIN";
    const operation = String(form.get("operation") || "add") as
      "add" | "remove";
    if (
      !["MEMBER", "ADMIN"].includes(requestedRole) ||
      !["add", "remove"].includes(operation)
    )
      throw new Error("طلب إدارة العضوية غير صالح.");

    const [actorMembershipRows, targetProfiles] = await Promise.all([
      supabaseFetch(
        `/rest/v1/organization_members?organization_id=eq.${encodeURIComponent(organizationId)}&user_id=eq.${encodeURIComponent(actor.id)}&select=role,organizations(name)&limit=1`,
      ),
      supabaseFetch(
        `/rest/v1/profiles?email=eq.${encodeURIComponent(memberEmail)}&select=id,email&limit=1`,
      ),
    ]);
    const actorMembership = actorMembershipRows?.[0] as
      | {
          role?: string;
          organizations?: { name?: string } | Array<{ name?: string }>;
        }
      | undefined;
    const organization = Array.isArray(actorMembership?.organizations)
      ? actorMembership?.organizations[0]
      : actorMembership?.organizations;
    const authorization = await authorizeOrganizationAction({
      internalAllowed: ["OWNER", "ADMIN"].includes(
    ...
    if (!authorization.allowed)
      throw new Error("ليست لديك صلاحية إدارة أعضاء مساحة العمل.");

    const target = targetProfiles?.[0] as
      { id?: string; email?: string } | undefined;
    let existingRole: "OWNER" | "ADMIN" | "MEMBER" | undefined;
    if (target?.id) {
      const targetMemberships = await supabaseFetch(
        `/rest/v1/organization_members?organization_id=eq.${encodeURIComponent(organizationId)}&user_id=eq.${encodeURIComponent(target.id)}&select=role&limit=1`,
      );
      const value = String(targetMemberships?.[0]?.role || "");
      if (["OWNER", "ADMIN", "MEMBER"].includes(value))
        existingRole = value as typeof existingRole;
    }
    ...
    if (target?.id) {
      const synchronizedRole =
        operation === "remove" ? existingRole || requestedRole : requestedRole;
      after(async () => {
    ...
    return {
      success: operation === "add" ? "تمت إضافة العضو." : "تمت إزالة العضو.",
    };
  } catch (error) {
    return {
      error: error instanceof Error ? error.message : "تعذر تحديث العضوية.",
    };
  }
}
