...
export default async function WorkspacePaymentPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{success?:string;error?:string}>}){
 ...
 const request=(await supabaseFetch(`/rest/v1/workspace_requests?id=eq.${encodeURIComponent(id)}&select=id,name,type,status,service_code,request_kind,onboarding_state,payment_reference,rejection_reason,organization_id,subscription_plans(id,name,description,price,currency,billing_months)`))?.[0];
 if(!request)notFound();
 if(request.status==='approved'&&request.onboarding_state==='ACTIVE')redirect('/account');
 ...
 return <main className="mx-auto max-w-3xl p-6 py-12">
  <p className="font-bold text-[#70E4D4]">{request.request_kind==='RENEWAL'?'تجديد خدمة':'تفعيل خدمة مَدار'}</p><h1 className="mt-2 text-3xl font-black">{request.name}</h1>
  ...
  <div className="mt-6 rounded-3xl border border-white/10 bg-white/[.04] p-6">
   ...
   {request.status==='pending_review'&&<div className="mt-5 rounded-2xl border border-amber-300/20 bg-amber-300/5 p-5 leading-8 text-amber-100">وصل إثبات الدفع ومرجع العملية <strong>{request.payment_reference||'—'}</strong>. لن تُفتح الخدمة قبل اعتماد الإدارة.</div>}
   {request.status==='approved'&&request.onboarding_state==='PROVISIONING'&&<div className="mt-5 rounded-2xl border border-violet-300/20 bg-violet-300/5 p-5 leading-8 text-violet-100">اعتمدت الإدارة الدفع، ويجري تجهيز مساحة MADAR Retail المعزولة. لا وصول مالي قبل اكتمال التهيئة.</div>}
   ...
   <Link href="/account" className="mt-6 inline-block text-sm font-bold text-[#70E4D4]">العودة إلى حسابي ←</Link>
  </div>
 </main>;
}
