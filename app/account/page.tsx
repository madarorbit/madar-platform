...
export default async function AccountPageRoute({ searchParams }: { searchParams: Promise<{ error?: string; service?: string; view?: string }> }) {
  ...
  const expiryAttention = activeServices
    ...
    .map((service): AttentionItem => ({ key: `expiry-${service.definition.code}`, title: `اشتراك ${service.definition.shortName} يقترب من الانتهاء`, description: `ينتهي في ${formatDate(service.subscription!.ends_at)}.`, href: "/account/subscriptions", tone: "warning" }));
  const storeAttention = data.orders.data
    .filter((order) => ["unpaid", "rejected"].includes(order.payment_status))
    .map((order): AttentionItem => ({ key: `order-${order.id}`, title: order.payment_status === "rejected" ? "راجع دفعة طلب المتجر" : "أكمل دفع طلب المتجر", description: `الطلب ${order.order_number} يحتاج إجراءً قبل اكتماله.`, href: `/account/orders/${order.id}`, tone: order.payment_status === "rejected" ? "danger" : "warning" }));
  const plusAttention: AttentionItem[] = data.plusPayment.data?.status === "rejected" ? [{ key: `orby-payment-${data.plusPayment.data.id}`, title: "راجع طلب ORBY Plus", description: data.plusPayment.data.review_note || "تعذر اعتماد الدفعة الحالية.", href: "/orby/plus", tone: "danger" }] : [];
  ...
  return (
    <AccountPage>
      ...
      <nav className="md-home-quick-access md-account-quick-access" aria-label="وصول سريع"><QuickLink href="/account/payments" icon="document" label="مدفوعاتي" /><QuickLink href="/account/purchases" icon="briefcase" label="مكتبتي" /><QuickLink href="/account/orby" icon="sparkles" label="خطة ORBY" /><QuickLink href="/account/profile" icon="user" label="الملف الشخصي" /></nav>

      <details className="md-account-secondary">
        <summary><span><strong>تفاصيل الحساب والنشاط</strong><small>الاشتراكات، المكتبة وآخر التحديثات</small></span><Icon name="arrow" className="md-account-secondary-arrow" /></summary>
        <div className="md-account-secondary-body">
