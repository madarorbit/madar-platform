...
const routes=['','store','products','services','subscriptions','categories','offers','featured','latest','best-sellers','free','search','about','blog','docs','help','community','careers','contact','faq','privacy','terms','refund-policy','service-agreement','login','register'];
...
export default async function sitemap():Promise<MetadataRoute.Sitemap>{
 ...
 return [
  ...routes.map(route=>({url:`${siteConfig.baseUrl}/${route}`.replace(/\/$/,'/'),lastModified,changeFrequency:route==='blog'?'weekly' as const:'monthly' as const,priority:route?0.7:1})),
