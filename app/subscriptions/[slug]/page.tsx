...
export async function generateMetadata({params}:Props):Promise<Metadata>{
 ...
 try{
  const item=await getStoreItem('plan',slug);
  if(!item)return{title:'اشتراك غير موجود',robots:{index:false,follow:false}};
  const url=`${siteConfig.baseUrl}/subscriptions/${slug}`;
 ...
 }catch{return{title:'تعذر تحميل الاشتراك',robots:{index:false,follow:false}}}
}
