...
import OrbyFloatingFace from '@/components/orby/OrbyFloatingFace';

const metadataBase=new URL(siteConfig.baseUrl);
const googleAnalyticsId='G-PT7RKF7295';
const themeBootstrap=`(()=>{try{const raw=localStorage.getItem('madar-theme');const preference=raw==='light'||raw==='dark'||raw==='system'?raw:'system';const theme=preference==='system'?(matchMedia('(prefers-color-scheme: dark)').matches?'dark':'light'):preference;const root=document.documentElement;root.dataset.theme=theme;root.dataset.themePreference=preference;root.dataset.designSystem='2';root.style.colorScheme=theme}catch{const root=document.documentElement;root.dataset.theme='dark';root.dataset.themePreference='system';root.dataset.designSystem='2'}})();`;
