...
const requestAccessToken=(request:Request)=>{const header=request.headers.get('authorization')||'';const[scheme,token]=header.split(/\s+/,2);return scheme?.toLowerCase()==='bearer'&&token?token:undefined;};
function businessContextSource(mode:OrbyMode,context:OrbyContext):OrbyContextSource{return{key:'madar.business-context',priority:100,async load(){return{key:'madar.business-context',title:'سياق الأعمال الموثق من مَدار',content:JSON.stringify({task:orbyModes[mode],businessContext:context,sourceOfTruth:context.activity?.source_of_truth||'MADAR',generatedAt:context.analytics.generated_at}),priority:100,trusted:false,sensitive:true,metadata:{source:'madar-read-only-analytics',mode,generatedAt:context.analytics.generated_at}};}};}
async function legacyKernelSession(fetcher:SupabaseFetcher,organizationId:string,userId:string,conversationId:string|null){if(!conversationId||!uuidPattern.test(conversationId))return undefined;const rows=await fetcher(`/rest/v1/orby_messages?conversation_id=eq.${encodeURIComponent(conversationId)}&organization_id=eq.${encodeURIComponent(organizationId)}&user_id=eq.${encodeURIComponent(userId)}&role=eq.assistant&select=metadata&order=created_at.desc,id.desc&limit=20`) as LegacyMessageMetadata[];const value=rows.map(row=>row.metadata?.kernel_session_id).find(item=>typeof item==='string'&&uuidPattern.test(item));return typeof value==='string'&&uuidPattern.test(value)?value:undefined;}

async function executeOrbyCore(input:{organizationId:string;userId:string;sessionId?:string;mode:OrbyMode;prompt:string;context:OrbyContext;dialogue:DialogueDecision}):Promise<OrbyKernelResponse>{
 const foundation=await createServerOrbyFoundation({contextSources:[businessContextSource(input.mode,input.context)],configuration:{systemPolicies:[...input.dialogue.systemPolicies,orbySystemPrompt()]}});
 const request={identity:{organizationId:input.organizationId,userId:input.userId,workspaceId:input.organizationId},sessionId:input.sessionId,message:`استراتيجية الرد: ${input.dialogue.strategy}.\nنية الطلب: ${input.dialogue.classification.intent}.\nالمهمة المطلوبة: ${orbyModes[input.mode]}\n\nطلب المستخدم:\n${input.prompt}`,requiredCapabilities:['text'] as const,metadata:{purpose:'orby-business',mode:input.mode,promptVersion:input.dialogue.promptVersion,intent:input.dialogue.classification.intent,sector:input.dialogue.classification.sector,sensitivity:input.dialogue.classification.sensitivity}};
 try{return await foundation.kernel.execute(request);}
 catch(error){if(input.sessionId&&isOrbyError(error)&&(error.code==='SESSION_NOT_FOUND'||error.code==='SESSION_CLOSED'))return foundation.kernel.execute({...request,sessionId:undefined});throw error;}
}
