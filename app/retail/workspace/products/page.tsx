    ...
    <div className="content-grid">
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_390px]">
        <div className="table-wrap">
          <table className="data-table"><thead><tr><th>المنتج</th><th>التصنيف</th><th>البيع</th><th>متوسط التكلفة</th><th>المخزون</th><th>التنبيه</th><th>الحالة</th></tr></thead>
            <tbody>{products.map((product) => <tr key={product.id}><td><strong>{product.name}</strong><small className="muted block">{product.sku ?? product.barcode ?? "دون رمز"}</small></td><td>{relationName(product.categories)}</td><td>{formatMoney(product.sale_price, workspace.currency)}</td><td>{formatMoney(product.average_cost, workspace.currency)}</td><td>{formatQuantity(product.stock_on_hand)} {product.unit}</td><td>{formatQuantity(product.minimum_stock)}</td><td><span className={product.stock_on_hand <= product.minimum_stock ? "status-pill status-warning" : "status-pill"}>{product.status === "active" ? "نشط" : "متوقف"}</span></td></tr>)}</tbody>
          </table>
