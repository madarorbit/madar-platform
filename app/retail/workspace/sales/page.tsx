...
export default async function SalesPage({ searchParams }: { searchParams: Promise<{ success?: string; error?: string }> }) {
  ...
  return <div className="content-grid"><FlashMessage {...params} /><PageHeader eyebrow="عملية ذرّية" title="المبيعات" description="إتمام البيع يرحّل الفاتورة والمخزون والصندوق وذمة العميل معًا، أو يتراجع عنها كلها." />
    {role !== "VIEWER" ? <DocumentComposer mode="sale" products={products.filter((product) => product.status === "active")} parties={customers.map(({ id, name }) => ({ id, name }))} currency={workspace.currency} operationId={crypto.randomUUID()} /> : null}
    <section className="surface p-5"><h2 className="text-lg font-black">آخر الفواتير</h2><div className="table-wrap mt-4"><table className="data-table" data-mobile="list"><thead><tr><th>الفاتورة</th><th>العميل</th><th>الوقت</th><th>الدفع</th><th>المدفوع</th><th>الإجمالي</th></tr></thead><tbody>{sales.map((sale) => <tr key={sale.id}><td data-label="الفاتورة"><strong>{sale.invoice_number}</strong></td><td data-label="العميل">{relationName(sale.customers, "بيع مباشر")}</td><td data-label="الوقت">{formatDateTime(sale.sold_at)}</td><td data-label="حالة الدفع"><span className="status-pill">{sale.payment_status}</span></td><td data-label="المدفوع">{formatMoney(sale.amount_paid, workspace.currency)}</td><td data-label="الإجمالي">{formatMoney(sale.total, workspace.currency)}</td></tr>)}</tbody></table>{!sales.length ? <p className="muted p-8 text-center">لا توجد فواتير بعد.</p> : null}</div></section>
  </div>;
}
