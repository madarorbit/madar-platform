...
export default async function OrbyPage() {
  ...
  const { data: conversation } = await supabase.from("retail_orby_conversations").select("id").eq("workspace_id", workspace.id).eq("created_by", user.id).is("archived_at", null).order("updated_at", { ascending: false }).limit(1).maybeSingle();
  const { data: history } = conversation ? await supabase.from("retail_orby_messages").select("id,role,content,evidence").eq("workspace_id", workspace.id).eq("conversation_id", conversation.id).order("created_at").limit(40) : { data: [] };
