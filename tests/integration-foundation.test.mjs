...
test("connector SDK is isolated behind a versioned compile-time registry", async () => {
  const [contracts, registry, runtime] = await Promise.all([
    read("src/lib/integration/contracts.ts"),
    read("src/lib/integration/registry.ts"),
...
test("authentication strategies keep secrets server-side and support common schemes", async () => {
  const [auth, env, route] = await Promise.all([
    read("src/lib/integration/auth.ts"),
    read("src/lib/env.ts"),
  ...
  for (const scheme of [
    ...
    assert.match(auth, new RegExp(`${scheme}:`));
...
test("queue and sync engine provide leases, backoff, checkpoints and idempotency", async () => {
  const [platform, engine, errors, migration] = await Promise.all([
    read("src/lib/integration/platform.ts"),
    read("src/lib/integration/sync-engine.ts"),
    read("src/lib/integration/errors.ts"),
    read("supabase/migrations/20260727213000_integration_foundation_core.sql"),
  ]);
...
test("tenant isolation and secret denial are explicit in the migration", async () => {
  const migration = await read(
    "supabase/migrations/20260727213000_integration_foundation_core.sql",
  );
  ...
  assert.match(
    migration,
    /alter table public\.integration_connection_secrets enable row level security/i,
  );
  assert.doesNotMatch(migration, /policy .*integration_connection_secrets/i);
  assert.match(
    migration,
    /revoke all on public\.integration_connectors,public\.integration_connections,public\.integration_connection_secrets/i,
  );
  assert.match(
    migration,
    /grant execute on function public\.integration_claim_jobs.*to service_role/i,
  );
});

test("connection secret rotation is atomic and serialized per connection", async () => {
  const [manager, migration] = await Promise.all([
    read("src/lib/integration/connection-manager.ts"),
    read(
      "supabase/migrations/20260727215500_integration_secret_rotation_rpc.sql",
    ),
  ]);
  const start = manager.indexOf("async rotateSecret"),
    end = manager.indexOf("async enqueueSync");
  ...
  assert.match(
    migration,
    /update public\.integration_connection_secrets[\s\S]*insert into public\.integration_connection_secrets/i,
  );
  assert.match(
    migration,
    /grant execute on function public\.integration_rotate_connection_secret.*to service_role/i,
  );
});

test("write operations remain disabled by feature flag and read-only defaults", async () => {
  const [migration, contracts, manager] = await Promise.all([
    read("supabase/migrations/20260727213000_integration_foundation_core.sql"),
    read("src/lib/integration/contracts.ts"),
    read("src/lib/integration/connection-manager.ts"),
  ]);
  ...
  assert.match(manager, /connection\.status\s*!==\s*["']active["']/);
});
