...
export async function proxy(request:NextRequest){
 ...
 if(refresh){
  ...
  if(base&&key){
   ...
   if(response?.ok){const session=await response.json() as {access_token:string;refresh_token:string;expires_in:number};const result=NextResponse.next();result.cookies.set('madar-access-token',session.access_token,cookieOptions(session.expires_in||3600));result.cookies.set('madar-refresh-token',session.refresh_token,cookieOptions(SESSION_MAX_AGE));rememberPath(result,request);return result;}
  }
