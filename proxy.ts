...
export async function proxy(request:NextRequest){
 ...
 if(refresh){
  ...
  if(base&&key){const response=await fetch(`${base}/auth/v1/token?grant_type=refresh_token`,{method:'POST',headers:{apikey:key,'Content-Type':'application/json'},body:JSON.stringify({refresh_token:refresh}),cache:'no-store'}).catch(()=>null);if(response?.ok){const session=await response.json() as {access_token:string;refresh_token:string;expires_in:number};return refreshedResponse(request,session);}}
