-- MADAR Retail V0 — unified product foundation inside MADAR Platform.
-- Every Retail table is namespaced with retail_ to avoid collisions while
-- keeping one Supabase Auth identity and one operational database.
...
create or replace function private.retail_touch_updated_at()
returns trigger
language plpgsql
security invoker
set search_path = ''
...
revoke all on function private.retail_touch_updated_at() from public, anon, authenticated;

create or replace function private.retail_touch_versioned_updated_at()
returns trigger
language plpgsql
security invoker
set search_path = ''
...
revoke all on function private.retail_touch_versioned_updated_at() from public, anon, authenticated;
...
create index retail_profiles_role_status_idx on public.retail_profiles(platform_role, status);
...
alter table public.retail_profiles
  add constraint retail_profiles_active_workspace_fk
  foreign key (active_workspace_id) references public.retail_workspaces(id) on delete set null;

create table public.retail_workspace_members (
...
create index retail_workspace_members_user_idx on public.retail_workspace_members(user_id, status);

create table public.retail_onboarding_drafts (
  user_id uuid primary key references public.retail_profiles(id) on delete cascade,
...
alter table public.retail_onboarding_drafts
  add constraint retail_onboarding_drafts_plan_fk
  foreign key (selected_plan_id) references public.retail_plans(id) on delete set null;

create table public.retail_subscriptions (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.retail_workspaces(id) on delete cascade,
  plan_id uuid not null references public.retail_plans(id) on delete restrict,
  ...
  approved_by uuid references public.retail_profiles(id) on delete set null,
...
create index retail_subscriptions_status_end_idx on public.retail_subscriptions(status, ends_at);

create table public.retail_payment_methods (
  id uuid primary key default gen_random_uuid(),
...
create table public.retail_payment_requests (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.retail_workspaces(id) on delete cascade,
  subscription_id uuid references public.retail_subscriptions(id) on delete set null,
  plan_id uuid not null references public.retail_plans(id) on delete restrict,
  payment_method_id uuid not null references public.retail_payment_methods(id) on delete restrict,
  requested_by uuid not null references public.retail_profiles(id) on delete restrict,
  ...
  reviewed_by uuid references public.retail_profiles(id) on delete set null,
...
create unique index retail_payment_requests_one_pending_idx
  on public.retail_payment_requests(workspace_id)
  where status = 'under_review';
create index retail_payment_requests_review_queue_idx
  on public.retail_payment_requests(status, created_at);

create table public.retail_audit_logs (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid references public.retail_workspaces(id) on delete cascade,
  actor_id uuid references public.retail_profiles(id) on delete set null,
...
create index retail_audit_logs_workspace_created_idx on public.retail_audit_logs(workspace_id, created_at desc);
create index retail_audit_logs_action_created_idx on public.retail_audit_logs(action, created_at desc);
...
create unique index retail_categories_workspace_name_unique
  on public.retail_categories(workspace_id, lower(name))
...
create table public.retail_products (
  ...
  foreign key (workspace_id, category_id)
    references public.retail_categories(workspace_id, id) on delete restrict
);

create unique index retail_products_workspace_sku_unique
  on public.retail_products(workspace_id, lower(sku))
  where sku is not null and btrim(sku) <> '' and deleted_at is null;
create unique index retail_products_workspace_barcode_unique
  on public.retail_products(workspace_id, barcode)
  where barcode is not null and btrim(barcode) <> '' and deleted_at is null;
create index retail_products_workspace_active_idx on public.retail_products(workspace_id, status, name)
  where deleted_at is null;
create index retail_products_low_stock_idx on public.retail_products(workspace_id, stock_on_hand, minimum_stock)
...
create index retail_customers_workspace_name_idx on public.retail_customers(workspace_id, name)
  where deleted_at is null;
create index retail_customers_workspace_phone_idx on public.retail_customers(workspace_id, phone)
...
create index retail_suppliers_workspace_name_idx on public.retail_suppliers(workspace_id, name)
  where deleted_at is null;

create trigger profiles_updated before update on public.retail_profiles
...
create trigger workspace_members_updated before update on public.retail_workspace_members
for each row execute function private.retail_touch_updated_at();
create trigger onboarding_drafts_updated before update on public.retail_onboarding_drafts
for each row execute function private.retail_touch_updated_at();
create trigger plans_updated before update on public.retail_plans
for each row execute function private.retail_touch_versioned_updated_at();
create trigger subscriptions_updated before update on public.retail_subscriptions
for each row execute function private.retail_touch_versioned_updated_at();
create trigger payment_methods_updated before update on public.retail_payment_methods
for each row execute function private.retail_touch_versioned_updated_at();
create trigger payment_requests_updated before update on public.retail_payment_requests
for each row execute function private.retail_touch_versioned_updated_at();
create trigger categories_updated before update on public.retail_categories
for each row execute function private.retail_touch_versioned_updated_at();
create trigger products_updated before update on public.retail_products
for each row execute function private.retail_touch_versioned_updated_at();
create trigger customers_updated before update on public.retail_customers
for each row execute function private.retail_touch_versioned_updated_at();
create trigger suppliers_updated before update on public.retail_suppliers
for each row execute function private.retail_touch_versioned_updated_at();

insert into public.retail_plans(
...
values (
  ...
  '{"members":1,"retail_products":500,"orby_daily_requests":30}'::jsonb
);
