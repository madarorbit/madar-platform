-- MADAR Retail V0 — atomic retail_sales, retail_purchases, debts, collections and returns.
...
declare
  ...
  result jsonb;
begin
  existing := private.retail_begin_operation(target_workspace, actor, target_operation, 'SALE_CREATE', source_device, nullif(payload->>'client_created_at', '')::timestamptz);
  ...
  if customer_value is not null and not exists (
    select 1 from public.retail_customers
  ...
  if due_value > 0 then
    insert into public.retail_receivables(
    ...
    insert into public.retail_debt_transactions(
  ...
  perform private.retail_finish_operation(target_workspace, target_operation, 'sale', sale_id, result);
  perform private.retail_write_audit(target_workspace, actor, 'sale.created', 'sale', sale_id, target_operation, jsonb_build_object('invoice_number', invoice_number_value, 'total', total_value, 'balance_due', due_value));
  return result;
end;
$$;
...
declare
  ...
  result jsonb;
begin
  existing := private.retail_begin_operation(target_workspace, actor, target_operation, 'PURCHASE_CREATE', source_device, nullif(payload->>'client_created_at', '')::timestamptz);
  if existing is not null then return existing; end if;
  if jsonb_typeof(payload->'items') <> 'array'
    or jsonb_array_length(payload->'items') < 1
    or jsonb_array_length(payload->'items') > 100 then
  ...
  if supplier_value is not null and not exists (
    select 1 from public.retail_suppliers
  ...
  if due_value > 0 then
    insert into public.retail_payables(
  ...
  perform private.retail_finish_operation(target_workspace, target_operation, 'purchase', purchase_id, result);
  perform private.retail_write_audit(target_workspace, actor, 'purchase.created', 'purchase', purchase_id, target_operation, jsonb_build_object('purchase_number', purchase_number_value, 'total', total_value, 'balance_due', due_value));
  return result;
end;
$$;
...
declare
  actor uuid := private.retail_require_workspace_actor(target_workspace);
  existing jsonb;
  receivable public.retail_receivables%rowtype;
  ...
  result jsonb;
begin
  existing := private.retail_begin_operation(target_workspace, actor, target_operation, 'RECEIVABLE_COLLECTION', source_device, nullif(payload->>'client_created_at', '')::timestamptz);
  if existing is not null then return existing; end if;
  ...
  update public.retail_receivables
  ...
  perform private.retail_finish_operation(target_workspace, target_operation, 'debt_transaction', transaction_id, result);
  perform private.retail_write_audit(target_workspace, actor, 'receivable.collected', 'receivable', receivable.id, target_operation, jsonb_build_object('amount', amount_value, 'remaining_due', next_due));
  return result;
end;
$$;
...
declare
  actor uuid := private.retail_require_workspace_actor(target_workspace);
  existing jsonb;
  payable public.retail_payables%rowtype;
  amount_value numeric(18,2) := nullif(payload->>'amount', '')::numeric;
  method text := coalesce(payload->>'payment_method', 'CASH');
  next_due numeric(18,2);
  party_balance numeric(18,2);
  ...
  result jsonb;
begin
  existing := private.retail_begin_operation(target_workspace, actor, target_operation, 'PAYABLE_PAYMENT', source_device, nullif(payload->>'client_created_at', '')::timestamptz);
  if existing is not null then return existing; end if;
  ...
  perform private.retail_finish_operation(target_workspace, target_operation, 'debt_transaction', transaction_id, result);
  perform private.retail_write_audit(target_workspace, actor, 'payable.paid', 'payable', payable.id, target_operation, jsonb_build_object('amount', amount_value, 'remaining_due', next_due));
  return result;
end;
$$;
...
declare
  actor uuid := private.retail_require_workspace_actor(target_workspace);
  existing jsonb;
  sale public.retail_sales%rowtype;
  sale_item public.retail_sale_items%rowtype;
  product public.retail_products%rowtype;
  receivable public.retail_receivables%rowtype;
  item jsonb;
  quantity_value numeric(18,3);
  ...
  result jsonb;
begin
  existing := private.retail_begin_operation(target_workspace, actor, target_operation, 'SALE_RETURN', source_device, nullif(payload->>'client_created_at', '')::timestamptz);
  if existing is not null then return existing; end if;
  ...
  return_number_value := sale.invoice_number || '-R' || (
    select (count(*) + 1)::text from public.retail_sale_returns
    where workspace_id = target_workspace and sale_id = sale.id
  );
  insert into public.retail_sale_returns(
  ...
  loop
    ...
    insert into public.retail_sale_return_items(
  ...
  perform private.retail_finish_operation(target_workspace, target_operation, 'sale_return', return_id, result);
  perform private.retail_write_audit(target_workspace, actor, 'sale.returned', 'sale_return', return_id, target_operation, jsonb_build_object('sale_id', sale.id, 'refund_total', refund_total_value));
  return result;
end;
$$;
...
declare
  ...
  -- mutations still use retail_require_workspace_actor and remain blocked.
  actor uuid := private.retail_require_membership_actor(target_workspace, array['OWNER']::text[]);
  existing jsonb;
  selected_plan public.retail_plans%rowtype;
  selected_method public.retail_payment_methods%rowtype;
  subscription_id_value uuid;
  request_id uuid;
  amount_value numeric(18,2) := nullif(payload->>'amount', '')::numeric;
  result jsonb;
begin
  existing := private.retail_begin_operation(target_workspace, actor, target_operation, 'PAYMENT_REQUEST_SUBMIT', null, null);
  if existing is not null then return existing; end if;
  select * into selected_plan from public.retail_plans where id = (payload->>'plan_id')::uuid and status = 'active' and is_public;
  select * into selected_method from public.retail_payment_methods where id = (payload->>'payment_method_id')::uuid and status = 'active';
  ...
  select id into subscription_id_value from public.retail_subscriptions where workspace_id = target_workspace;

  insert into public.retail_payment_requests(
  ...
  perform private.retail_finish_operation(target_workspace, target_operation, 'payment_request', request_id, result);
  perform private.retail_write_audit(target_workspace, actor, 'payment_request.submitted', 'payment_request', request_id, target_operation, jsonb_build_object('plan_id', selected_plan.id, 'amount', amount_value));
  return result;
end;
$$;
...
declare
  ...
  request public.retail_payment_requests%rowtype;
  selected_plan public.retail_plans%rowtype;
  new_end timestamptz;
  new_grace_end timestamptz;
  result jsonb;
begin
  if actor is null or not private.retail_is_platform_admin() then raise exception 'ADMIN_REQUIRED'; end if;
  if decision not in ('approve', 'reject') then raise exception 'INVALID_DECISION'; end if;
  select * into request from public.retail_payment_requests where id = target_request for update;
  if request.id is null or request.status <> 'under_review' then raise exception 'PAYMENT_NOT_REVIEWABLE'; end if;

  if decision = 'approve' then
    select * into selected_plan from public.retail_plans where id = request.plan_id and status = 'active';
    ...
    new_end := greatest(
      coalesce((select ends_at from public.retail_subscriptions where workspace_id = request.workspace_id), now()),
      coalesce((select trial_ends_at from public.retail_subscriptions where workspace_id = request.workspace_id), now()),
      now()
    )
    ...
    update public.retail_payment_requests
  ...
  perform private.retail_write_audit(request.workspace_id, actor, 'payment_request.' || case when decision = 'approve' then 'approved' else 'rejected' end, 'payment_request', request.id, gen_random_uuid(), jsonb_build_object('review_note_present', note is not null));
  return result;
end;
$$;
...
begin
  ...
  perform private.retail_write_audit(target_workspace, actor, 'workspace.' || target_status, 'workspace', target_workspace, gen_random_uuid(), jsonb_build_object('note', nullif(btrim(note), '')));
