...
declare actor uuid := (select auth.uid());
begin
  if actor is null or not private.retail_is_workspace_member(target_workspace) then
    raise exception 'NOT_AUTHORIZED';
  end if;
  update public.retail_profiles set active_workspace_id = target_workspace where id = actor;
...
set search_path = ''
as $$
declare actor uuid := private.retail_require_workspace_actor(target_workspace, array['OWNER', 'MANAGER']::text[]);
begin
  ...
  perform private.retail_write_audit(target_workspace, actor, 'workspace.settings_updated', 'workspace', target_workspace, gen_random_uuid(), '{}'::jsonb);
  return jsonb_build_object('workspace_id', target_workspace);
end;
$$;
...
declare
  ...
  result jsonb;
begin
  existing := private.retail_begin_operation(target_workspace, actor, target_operation, 'CATEGORY_UPSERT', source_device, nullif(payload->>'client_created_at', '')::timestamptz);
  ...
  if target_id is null then
    insert into public.retail_categories(workspace_id, name, created_by)
    values(target_workspace, btrim(payload->>'name'), actor) returning id into target_id;
  else
    update public.retail_categories set name = btrim(payload->>'name'), deleted_at = null
    where workspace_id = target_workspace and id = target_id;
    if not found then raise exception 'CATEGORY_NOT_FOUND'; end if;
  end if;
  result := jsonb_build_object('category_id', target_id);
  perform private.retail_finish_operation(target_workspace, target_operation, 'category', target_id, result);
  perform private.retail_write_audit(target_workspace, actor, 'category.upserted', 'category', target_id, target_operation, '{}'::jsonb);
  return result;
end;
$$;
...
declare
  ...
  result jsonb;
begin
  existing := private.retail_begin_operation(target_workspace, actor, target_operation, 'PRODUCT_UPDATE', source_device, nullif(payload->>'client_created_at', '')::timestamptz);
  if existing is not null then return existing; end if;
  select * into product from public.retail_products where workspace_id = target_workspace and id = target_id for update;
  ...
  perform private.retail_finish_operation(target_workspace, target_operation, 'product', target_id, result);
  perform private.retail_write_audit(target_workspace, actor, case when delete_requested then 'product.deleted' else 'product.updated' end, 'product', target_id, target_operation, '{}'::jsonb);
  return result;
end;
$$;
...
declare
  ...
  result jsonb;
begin
  existing := private.retail_begin_operation(target_workspace, actor, target_operation, 'CUSTOMER_UPSERT', source_device, nullif(payload->>'client_created_at', '')::timestamptz);
  if existing is not null then return existing; end if;
  if char_length(btrim(coalesce(payload->>'name', ''))) not between 1 and 160 then raise exception 'INVALID_CUSTOMER_NAME'; end if;
  if target_id is null then
    insert into public.retail_customers(workspace_id, name, phone, notes, created_by)
  ...
  perform private.retail_finish_operation(target_workspace, target_operation, 'customer', target_id, result);
  perform private.retail_write_audit(target_workspace, actor, 'customer.upserted', 'customer', target_id, target_operation, '{}'::jsonb);
  return result;
end;
$$;
...
declare
  ...
  result jsonb;
begin
  existing := private.retail_begin_operation(target_workspace, actor, target_operation, 'SUPPLIER_UPSERT', source_device, nullif(payload->>'client_created_at', '')::timestamptz);
  if existing is not null then return existing; end if;
  if char_length(btrim(coalesce(payload->>'name', ''))) not between 1 and 160 then raise exception 'INVALID_SUPPLIER_NAME'; end if;
  if target_id is null then
    insert into public.retail_suppliers(workspace_id, name, phone, notes, created_by)
    values(target_workspace, btrim(payload->>'name'), nullif(btrim(payload->>'phone'), ''), nullif(btrim(payload->>'notes'), ''), actor)
    returning id into target_id;
  else
    update public.retail_suppliers
    set name = btrim(payload->>'name'),
        phone = case when payload ? 'phone' then nullif(btrim(payload->>'phone'), '') else phone end,
        notes = case when payload ? 'notes' then nullif(btrim(payload->>'notes'), '') else notes end,
        status = case when payload ? 'status' then payload->>'status' else status end,
  ...
  perform private.retail_finish_operation(target_workspace, target_operation, 'supplier', target_id, result);
  perform private.retail_write_audit(target_workspace, actor, 'supplier.upserted', 'supplier', target_id, target_operation, '{}'::jsonb);
  return result;
end;
$$;
...
begin
  ...
  if target_id is null then
    insert into public.retail_plans(code, name_ar, description_ar, status, is_public, price_amount, currency, billing_months, trial_days, grace_days, features, limits)
    ...
    update public.retail_plans set
  ...
  perform private.retail_write_audit(null, actor, 'plan.upserted', 'plan', target_id, gen_random_uuid(), '{}'::jsonb);
...
begin
  ...
  if conversation_id_value is null then
    insert into public.retail_orby_conversations(workspace_id, title, created_by)
    values(target_workspace, left(btrim(user_message), 120), actor)
    returning id into conversation_id_value;
  elsif not exists (
    select 1 from public.retail_orby_conversations
  ...
  insert into public.retail_orby_messages(
  ...
  update public.retail_orby_conversations set updated_at = now() where id = conversation_id_value;
...
begin
  ...
  join public.retail_subscriptions s on s.workspace_id = w.id
  join public.retail_plans p on p.id = s.plan_id
  where w.id = target_workspace;
  daily_limit := least(greatest(daily_limit, 0), 10000);

  insert into public.retail_orby_usage_daily(workspace_id, usage_date, request_count)
  values(target_workspace, local_day, 1)
  on conflict (workspace_id, usage_date) do update
  set request_count = public.retail_orby_usage_daily.request_count + 1,
      updated_at = now()
  where public.retail_orby_usage_daily.request_count < daily_limit
...
begin
  ...
  if target_id is null then
    insert into public.retail_payment_methods(code, name_ar, kind, account_name, account_identifier, instructions_ar, currency, status, sort_order)
    ...
    update public.retail_payment_methods set
  ...
  perform private.retail_write_audit(null, actor, 'payment_method.upserted', 'payment_method', target_id, gen_random_uuid(), '{}'::jsonb);
