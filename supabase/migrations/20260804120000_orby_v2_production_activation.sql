...
-- Initializes existing organizations and keeps vertical/source state synchronized
-- for future organizations, sector choices, subscriptions and connectors.
...
begin
  ...
  if org.id is null then return; end if;
  ...
  into connector_key_value,connector_synced_at
  from public.integration_connections c
  where c.organization_id=org.id
    and c.deleted_at is null
    and upper(c.status)='ACTIVE'
  order by c.last_success_at desc nulls last,c.updated_at desc
  limit 1;
  ...
  insert into public.orby_source_of_truth_states(
    ...
    org.id,org.operating_mode,org.source_of_truth,connector_key_value,
    connector_key_value is not null,connector_synced_at,write_allowlist,owner_id,now()
  )
  on conflict(organization_id) do update set
    operating_mode=excluded.operating_mode,
    source_of_truth=excluded.source_of_truth,
    connector_id=excluded.connector_id,
    connector_authorized=excluded.connector_authorized,
    last_synced_at=excluded.last_synced_at,
    allowed_write_operations=excluded.allowed_write_operations,
  ...
  if org.type='STUDENT' then
    insert into public.orby_vertical_installations(
      organization_id,vertical_key,plugin_version,plan_level,status,terminology,kpis,tool_allowlist,configuration,installed_by
    ) values
      (org.id,'personal','2.0.0','BASIC',case when org.status='active' then 'active' else 'paused' end,'{}'::jsonb,'[]'::jsonb,'[]'::jsonb,jsonb_build_object('adapter','personal','sharedKernel',true),owner_id),
      (org.id,'student','2.0.0','BASIC',case when org.status='active' then 'active' else 'paused' end,'{}'::jsonb,'[]'::jsonb,'[]'::jsonb,jsonb_build_object('adapter','student','studentSpaceVersion','1.3','sharedKernel',true),owner_id)
  ...
  order by osp.activated_at desc
  limit 1;

  if specialization.extension_key is null then return; end if;
  vertical:=case specialization.extension_key
  ...
  where t.extension_key=specialization.extension_key
  ...
  insert into public.orby_vertical_installations(
    ...
    org.id,vertical,'2.0.0',current_plan,
    case when org.status='active' then 'active' else 'paused' end,
    coalesce(specialization.terminology,'{}'::jsonb),coalesce(specialization.default_kpis,'[]'::jsonb),coalesce(tools,'[]'::jsonb),
  ...
  where organization_id=org.id and vertical_key<>vertical and vertical_key not in ('personal','student') and status='active';
end;
$$;
...
begin
  perform private.sync_orby_v2_organization_state(coalesce(new.id,old.id));
...
returns trigger language plpgsql security definer set search_path='' as $$
begin
  perform private.sync_orby_v2_organization_state(coalesce(new.organization_id,old.organization_id));
  return coalesce(new,old);
end;
$$;
...
create trigger organizations_orby_v2_sync
after insert or update of status,operating_mode,source_of_truth on public.organizations
...
create trigger activity_profiles_orby_v2_sync
after insert or update of status,specialization_id,operating_mode on public.activity_profiles
...
create trigger organization_sector_packages_orby_v2_sync
after insert or update or delete on public.organization_sector_packages
for each row execute function private.trigger_sync_orby_v2_scoped_record();

drop trigger if exists pricing_subscription_snapshots_orby_v2_sync on public.pricing_subscription_snapshots;
create trigger pricing_subscription_snapshots_orby_v2_sync
after insert or update of status,variant_id,locked_entitlements on public.pricing_subscription_snapshots
for each row execute function private.trigger_sync_orby_v2_scoped_record();

drop trigger if exists integration_connections_orby_v2_sync on public.integration_connections;
create trigger integration_connections_orby_v2_sync
after insert or update of status,last_success_at,connector_key,deleted_at or delete on public.integration_connections
for each row execute function private.trigger_sync_orby_v2_scoped_record();

do $$ declare org_id uuid;
begin
  for org_id in select id from public.organizations loop
    perform private.sync_orby_v2_organization_state(org_id);
  end loop;
end $$;
...
begin
  insert into public.orby_os_releases(
    organization_id,component,component_key,version,status,rollout_percentage,previous_version,metadata,created_at,activated_at,scope_key
  )
  select null,'core','orby-os','2.0.0','active',100,'1.0.0',
    jsonb_build_object(
      ...
      'acceptanceSuites',jsonb_build_array('repository-159+','orby-o1-o3','orby-o4-o7','orby-os-smoke','orby-os-security','orby-os-load','typescript','next-build')
    ),now(),now(),'global'
  where not exists(
    select 1 from public.orby_os_releases r where r.scope_key='global' and r.component='core' and r.component_key='orby-os' and r.version='2.0.0'
  );

  select id into release_id from public.orby_os_releases
  where scope_key='global' and component='core' and component_key='orby-os' and version='2.0.0'
  order by created_at desc limit 1;

  update public.orby_os_releases set status='deprecated'
  where scope_key='global' and component='core' and component_key='orby-os' and version='1.0.0' and id<>release_id;

  if not exists(select 1 from public.orby_release_gate_runs where release_id=release_id and core_version='2.0.0' and status='passed') then
    insert into public.orby_release_gate_runs(release_id,core_version,status,gate_results,score,artifact_refs,started_at,completed_at)
    values(
      release_id,'2.0.0','passed',
      jsonb_build_array(
      ...
      jsonb_build_array('GitHub CI','Enterprise transformation','Vercel build','Supabase production migrations'),now(),now()
    );
  end if;
end $$;
...
values(
  ...
  jsonb_build_object('orbyVersion','2.0.0','kernel','ORBY OS','externalWritesEnabled',false,'realPilotRequired',true,'activatedAt',now()),
  'approved',now()
)
on conflict(key) do update set release_version=excluded.release_version,decision=excluded.decision,status='approved',approved_at=excluded.approved_at,updated_at=now();
